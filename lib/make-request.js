  ...
  async function sendRequest (method, endpoint, post, cb) {
    ...
    if (post) {
      if (isAlert) {
        ...
        post.headers['content-length'] = Buffer.byteLength(JSON.stringify({data: {alertBody: post.data.alertBody}}));
      } else {
    ...
    if (post) {
      if (isAlert) {
        console.log('sending alert: ' + JSON.stringify({data: {alertBody: post.data.alertBody}}));
