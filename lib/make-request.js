...
module.exports = function (config) {
  function sendRequest (method, endpoint, post, cb, isAlert) {
    ...
    if (post) {
      if (isAlert) {
        console.log('alerting: ' + JSON.stringify({data: {alertBody: post.data.alertBody}}, null, 2));

        canonicalRequest = Buffer.concat([canonicalRequest, Buffer.from(JSON.stringify({data: {alertBody: post.data.alertBody}}), 'utf-8')]);
      } else {
    ...
    if (post) {
      if (isAlert) {
        req.write(JSON.stringify({data: {alertBody: post.data.alertBody}}));
      } else {
  ...
  return function makeRequest (method, endpoint, requestOpts, cb) {
    ...
    if (method === 'POST' && requestOpts.formData) {
      if (requestOpts.formData.data && requestOpts.formData.data.alertBody) {
        sendRequest(method, endpoint, requestOpts.formData, done, true);
      } else {
        return encodeFormData(requestOpts.formData || {}, function (err, encoded) {
          ...
          sendRequest(method, endpoint, encoded, done, false);
        });
      }
    }

    sendRequest(method, endpoint, null, done, false);
  };
};
